function rom = romberg(f,a,b,epsilon)
...
%   Integral = (1 - exp(-2*pi))/2 = 0.499066278634146 to 15 digits
...
% convert upper triangular r to lower triangular rom
for j = 1:step
    for i = 1:step
        if i>=j rom(i,j) = r(i-j+1,j);
        else rom(i,j) = 0;
        end
    end
end
